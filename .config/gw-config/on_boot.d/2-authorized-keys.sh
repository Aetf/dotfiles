...
# /data/gw-config/authorized_keys.d by deploy.sh. /root has survived every
...
# same reasoning as the nspawn restore in 0-setup-system.sh. Append-only
# and idempotent: keys added by hand on the box are left alone.

set -e

keydir=/data/gw-config/authorized_keys.d
[ -d "$keydir" ] || exit 0
